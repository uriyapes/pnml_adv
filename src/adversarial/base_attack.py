...
from adversarial.attacks import Adversarials
...
class BaseAttack(ABC):
    def __init__(self,
        ...
        self.save_adv_sample = save_adv_sample
        self.save_original_sample = save_original_sample
    ...
    def get_adversarial_object(self, x: torch.Tensor, y: Union[torch.Tensor, None], y_target: torch.Tensor = None) -> Adversarials:
        adv_sample, adv_loss, adv_pred, genie_pred = self.create_adversarial_sample(x, y, y_target)
        adv_sample = adv_sample if self.save_adv_sample else None
        original_sample = x if self.save_original_sample else None
