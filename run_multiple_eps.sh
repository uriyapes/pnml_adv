...
#    cuda_device=$((first_cuda_device_index + idx))
    cuda_device=$((first_cuda_device_index))
    printf "\ncuda_device=%s" "$cuda_device"
    CUDA_VISIBLE_DEVICES=$cuda_device python ./src/eval.py -t $exp_name -e ${test_eps_arr[idx]} -o results/mnist/bpda_model_300_iter_pgd_diff_eps &
    sleep 4
done
cuda_device=$((first_cuda_device_index + idx))
CUDA_VISIBLE_DEVICES=$cuda_device python ./src/eval.py -t $exp_name -e ${test_eps_arr[idx]} -o output/bpda_model_300_iter_pgd_diff_eps